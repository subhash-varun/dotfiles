local function get_jdtls()
    ...
    -- Detect operating system, windows uses win and macOS uses mac
    ...
    if launcher == "" or vim.fn.filereadable(lombok) == 0 then
        return nil, nil, nil
    end
...
local function get_bundles()
    ...
    bundles = vim.tbl_filter(function(item)
        if not item or item == "" then
            return false
        end

        -- These jars are not OSGi bundles and make jdtls bundle loading fail.
        if item:match("jacocoagent%.jar$") then
            return false
        end

        if item:match("com%.microsoft%.java%.test%.runner%-jar%-with%-dependencies%.jar$") then
            return false
        end
...
local function get_workspace(root_dir)
    ...
    local workspace_root = root_dir or vim.fn.getcwd()
    local project_name = vim.fn.fnamemodify(workspace_root, ":p:t")
    local project_hash = vim.fn.sha256(workspace_root):sub(1, 8)

    -- Add a hash so similarly named folders do not share one jdtls workspace.
    local workspace_dir = workspace_path .. project_name .. "-" .. project_hash
    return workspace_dir
end

local function get_java_bin()
    if vim.env.JAVA_HOME and vim.env.JAVA_HOME ~= "" then
        local java_from_home = vim.env.JAVA_HOME .. "/bin/java"
        if vim.fn.executable(java_from_home) == 1 then
            return java_from_home, vim.env.JAVA_HOME
        end
    end

    local java21_candidates = {
        "/usr/lib/jvm/java-21-openjdk/bin/java",
        "/usr/lib/jvm/java-21-openjdk-amd64/bin/java",
    }

    for _, path in ipairs(java21_candidates) do
        if vim.fn.executable(path) == 1 then
            return path, vim.fn.fnamemodify(path, ":h:h")
        end
    end
...
local function cleanup_corrupt_jdtls_cache(workspace_dir)
    local cache_files = {
        workspace_dir .. "/.metadata/.plugins/org.eclipse.jdt.core/nonChainingJarsCache",
        workspace_dir .. "/.metadata/.plugins/org.eclipse.jdt.core/invalidArchivesCache",
    }
...
local function setup_jdtls()
    ...
    if not launcher or not os_config or not lombok then
        vim.notify("JDTLS is not installed via Mason yet. Run :Mason and install jdtls.", vim.log.levels.ERROR)
        return
    end
    ...
    cleanup_corrupt_jdtls_cache(workspace_dir)
    ...
    local java_bin, java_home = get_java_bin()
    if java_home and java_home ~= "" then
        vim.env.JAVA_HOME = java_home
    end

    if java_bin == "java" then
        vim.notify("Using default java from PATH. If jdtls reports Java 21 errors, set JAVA_HOME to a JDK 21 install.", vim.log.levels.WARN)
    end
    ...
    local java_format_settings = nil
    local java_style_xml = vim.fn.stdpath("config") .. "/lang_servers/intellij-java-google-style.xml"
    if vim.fn.filereadable(java_style_xml) == 1 then
        java_format_settings = {
            url = java_style_xml,
            profile = "GoogleStyle"
        }
    end
